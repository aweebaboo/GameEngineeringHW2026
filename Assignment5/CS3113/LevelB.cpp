...
void LevelB::initialise()
{
   ...
   mGameState.bgm = LoadMusicStream("assets/game/Scarlet Forest.mp3");
   ...
   */
   ...
   std::map<Direction, std::vector<int>> enemyAnimAtlas = {
   ...
   mEnemies[0]->setAIType(SLIME);
   mEnemies[0]->setAIState(IDLE);
   mEnemies[0]->setEnemyHP(3);
   mEnemies[0]->setSpeed(30);
   mEnemies[0]->setColliderDimensions({48.0f, 48.0f});

   mEnemies[1] = new Entity(
      { mOrigin.x + 320.0f, mOrigin.y-200.0f },
      { 192.0f, 192.0f },
      "assets/game/slime.png",
      ATLAS,
      {4, 8},
   ...
   mEnemies[2]->setAIType(SLIME);
   mEnemies[2]->setAIState(IDLE);
   mEnemies[2]->setEnemyHP(3);
   mEnemies[2]->setSpeed(30);
   mEnemies[2]->setColliderDimensions({48.0f, 48.0f});

   mEnemies[3] = new Entity(
      { mOrigin.x + 450.0f, mOrigin.y + 250.0f },
      { 192.0f, 192.0f },
      "assets/game/slime.png",
      ATLAS,
      {4, 8},
   ...
   mEnemies[4]->setAIType(SLIME);
   mEnemies[4]->setAIState(IDLE);
   mEnemies[4]->setEnemyHP(3);
   mEnemies[4]->setSpeed(30);
   mEnemies[4]->setColliderDimensions({48.0f, 48.0f});

   mEnemies[5] = new Entity(
      { mOrigin.x + 100.0f, mOrigin.y + 400.0f },
      { 192.0f, 192.0f },
      "assets/game/slime.png",
      ATLAS,
      {4, 8},
   ...
   mEnemies[5]->setAIType(SLIME);
   mEnemies[5]->setAIState(IDLE);
   mEnemies[5]->setEnemyHP(3);
   mEnemies[5]->setSpeed(30);
   mEnemies[5]->setColliderDimensions({48.0f, 48.0f});

   mEnemies[6] = new Entity(
      { mOrigin.x - 150.0f, mOrigin.y - 350.0f },
      { 192.0f, 192.0f },
      "assets/game/slime.png",
      ATLAS,
      {4, 8},
   ...
   mEnemies[6]->setAIType(SLIME);
   mEnemies[6]->setAIState(IDLE);
   mEnemies[6]->setEnemyHP(3);
   mEnemies[6]->setSpeed(30);
   mEnemies[6]->setColliderDimensions({48.0f, 48.0f});

   mEnemies[7] = new Entity(
      { mOrigin.x + 550.0f, mOrigin.y - 50.0f },
      { 192.0f, 192.0f },
      "assets/game/slime.png",
      ATLAS,
      {4, 8},
   ...
   mEnemies[7]->setAIType(SLIME);
   mEnemies[7]->setAIState(IDLE);
   mEnemies[7]->setEnemyHP(3);
   mEnemies[7]->setSpeed(30);
   mEnemies[7]->setColliderDimensions({48.0f, 48.0f});
   ...
   mDialogueLines[0] = { "Old Man", "Oh no there are too many slimes here" };
   mDialogueLines[1] = { "Old Man", "We must kill them before they attack a nearby village" };
   mDialogueLines[2] = { "Old Man", "Let's make sure to kill all the slimes before we move on" };
   mDialogueCount = 3;
...
void LevelB::render()
{
   ...
   if (mGameState.door->isCollidingWith(mGameState.player) && enemiesKilled >= mEnemyCount)
   {
