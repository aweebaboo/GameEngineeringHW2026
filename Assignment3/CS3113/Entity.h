...
enum EntityType   { PLAYER, PLATFORM, FAIL, NONE};
...
public:
    ...
    void setAngle(float newAngle) 
        { mAngle = newAngle;                       }
    void setBoosting(bool setTo) 
        { mBoosting = setTo;                       }


    void turn(float degree) { mAngle += degree; }

    void boost(float deltaTime) {
        if (fuel > 0) {
            fuel -= fuel_use;
            mAcceleration.x = sinf(mAngle * DEG2RAD) * power;
            mAcceleration.y = -cosf(mAngle * DEG2RAD) * power + mGravity;
        } else {
            mAcceleration = { 0.0f, mGravity };
        }
    }

    void displayFuel() {
        DrawText(TextFormat("Fuel: %.0f", fuel), 20, 20, 20, WHITE);
        if (fuel < 0) fuel = 0;
    }
    ...
    void setFuelUse(float u) { fuel_use = u; }
