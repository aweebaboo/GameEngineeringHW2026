...
void initialise()
{
    ...
    gPlayer->setPower(70.0f);
    gPlayer->setFuelUse(0.08f);
    ...
    gWin->setColliderDimensions({ 100.0f, 30.0f });
...
void update()
{
    ...
    while (deltaTime >= FIXED_TIMESTEP)
    {
        ...
        if (playerPos.x < -100 || playerPos.x > SCREEN_WIDTH+100 || playerPos.y < -100 || playerPos.y > SCREEN_HEIGHT+100)
        {
            gPlayStatus = LOSS;
            gPlayer->deactivate();
        }
...
void render()
{
    ...
    if (gPlayStatus == LOSS && !gExplosion->isActive())
        DrawText("Mission Failed", SCREEN_WIDTH / 2 - 60, SCREEN_HEIGHT / 2, 30, RED);
    else if (gPlayStatus == WIN)
        DrawText("Mission Accomplished", SCREEN_WIDTH / 2 - 55, SCREEN_HEIGHT / 2, 30, GREEN);
